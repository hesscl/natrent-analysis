...
var_labels <- rev(c("Median HH Income", "% with College Degree", "% Non-Latino White",
                    "Median Gross Rent", "Median N Rooms", "% Single Family Home", "Median Owned HU Value",
                    "% Speaking English Only", "% Male", "% Age 65+", "% College Student", "% with Nonrelatives in HH",
                    "% in Same Home Last Year", "Distance to CBD", "Average HH Size", "% Age 20-34", "% Foreign Born",
                    "Population Density", "% HU Built Before 1940", "% Latino", "% Housing Cost Burdened", "% Non-Latino Black",
                    "Poverty Rate"))
...
#Extension 1: adding tensor smooth for vacancy * median year built
...
gam_lambda_form_2 <- log(lambda) ~ 
  ...
  trt_shr_blk + trt_shr_lat + 
  trt_shr_wht * log(trt_med_hh_inc) +
  dis_blk_wht * trt_shr_blk + 
  dis_lat_wht * trt_shr_lat
...
ggplot(gam_plot_vac, aes(x = trt_vac_rate, y = visregFit, 
  ...
  labs(x = "\nVacancy Rate", y = "Predicted log(Lambda)\n",
       fill = "Median Year Built", color = "Median Year Built") +
...
#plot the predictions, save to disk
ggplot(gam_plot_shr_blk, aes(x = trt_shr_blk, y = visregFit, 
...
#plot the predictions, save to disk
ggplot(gam_plot_shr_lat, aes(x = trt_shr_lat, y = visregFit, 
